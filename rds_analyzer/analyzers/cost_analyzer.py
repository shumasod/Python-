...
INSTANCE_HOURLY_RATES: dict[str, float] = {
    ...
    # Aurora 専用インスタンス（ap-northeast-1 推定値）
    # Aurora は RDS より約 5% 安価なケースが多い
    "db.r5.large.aurora": 0.285,
    "db.r5.xlarge.aurora": 0.570,
    "db.r5.2xlarge.aurora": 1.140,
    "db.r5.4xlarge.aurora": 2.280,
    "db.r6g.large.aurora": 0.242,
    "db.r6g.xlarge.aurora": 0.484,
    "db.r6g.2xlarge.aurora": 0.968,
    "db.r6g.4xlarge.aurora": 1.936,
    "db.m5.large.aurora": 0.203,
    "db.m5.xlarge.aurora": 0.406,
    "db.m5.2xlarge.aurora": 0.812,
    "db.m6g.large.aurora": 0.174,
    "db.m6g.xlarge.aurora": 0.347,
    "db.m6g.2xlarge.aurora": 0.694,
    "db.t3.medium.aurora": 0.082,
    "db.t4g.medium.aurora": 0.073,
}
...
# Aurora クラスターストレージ料金 (USD/GB-月)
# Aurora はストレージが自動スケールし、RDS gp2/gp3 とは別料金体系
AURORA_STORAGE_RATE = 0.11   # $0.11/GB-月（推定値）
AURORA_IO_RATE = 0.22        # I/O 最適化モード：$0.22/GB-月（I/O 料金込み）

# Aurora エンジン識別用セット
_AURORA_ENGINES = {EngineType.AURORA_MYSQL, EngineType.AURORA_POSTGRESQL}
...
class CostAnalyzer:
    ...
    def _compute_cost(
        ...
        """コンピュートコスト計算"""
        # Aurora エンジンは専用料金（.aurora サフィックス）を優先して使用
        aurora_key = f"{instance.instance_class}.aurora"
        if instance.engine in _AURORA_ENGINES and aurora_key in INSTANCE_HOURLY_RATES:
            hourly_rate = INSTANCE_HOURLY_RATES[aurora_key]
        else:
    ...
    def _storage_cost(
        ...
        """ストレージコスト計算"""
        # Aurora はストレージ体系が異なるため専用レートを使用
        if instance.engine in _AURORA_ENGINES:
            rate_per_gb = AURORA_STORAGE_RATE
        else:
